...
<header class="main-header">
  ...
  <nav class="navbar navbar-static-top">
    ...
    {!! Form::open(array('id' => 'change_default_company_form' ,'url' =>'admin/changeDefaultCompany', 'method'=>'post',
      'class' => 'form-horizontal form-validate', 'enctype'=>'multipart/form-data')) !!}
      <div class="navbar-custom-menu" style="float:left;">
        <ul class="nav navbar-nav">
          <li class="dropdown messages-menu">
            <div style="padding: 15px 15px;">
              @if(!empty(Session::get('owner_companies')) && count(Session::get('owner_companies')) >0 )
              <select name="company_id" id="company_id" class=""
                style="color:#000;-webkit-appearance: none;padding-left:5px;padding-right:5px;">
                @foreach (Session::get('owner_companies') as $own_company)
                <option value="{{$own_company->company_id}}" @if(Session::get('default_company_id'))
    ...
    <div class="navbar-custom-menu">
      <ul class="nav navbar-nav">
        ...
        <li class="dropdown user user-menu">
          <a href="#" class="dropdown-toggle" data-toggle="dropdown">
            <img src="{{asset('').auth()->guard('admin')->user()->image}}" class="user-image"
              alt="{{ auth()->guard('admin')->user()->first_name }} {{ auth()->guard('admin')->user()->last_name }} Image">
            <span class="hidden-xs">{{ auth()->guard('admin')->user()->first_name }}
          ...
          <ul class="dropdown-menu">
            ...
            <li class="user-footer">
              <div class="pull-left">
                <a href="{{ URL::to('admin/adminProfile')}}"
